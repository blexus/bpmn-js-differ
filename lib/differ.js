...
function trackAdditionalProperties(handler, a, b, key){
  var hitPolicyChangesLeft = findValues(a, key);
  if(hitPolicyChangesLeft.length == 0)
  {
    var hitPolicyChangesRight = findValues(b, key);

    if(hitPolicyChangesRight.length > 0){
      var diffModel = hitPolicyChangesRight[0];
      ...
      handler.changed(diffModel.model, diffModel.key, diffModel.value, defaultValue);
    } 
  }
}
...
function findValues(object, key) {
  ...
  function find(object, key){
    ...
    Object.keys(object).some(function(k) {
        if (k === key) {
            value = object[k];
            values.push({id: object['id'], value: value, model: object, key: key});
            return true;
        }
